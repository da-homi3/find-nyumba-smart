/**
 * Apply property_type enum expansion migration.
 * Usage: npm run db:migrate:property-types
 */
...
const MIGRATION_SQL = readFileSync(
  join(root, "supabase", "migrations", "20260706230000_property_type_categories.sql"),
  "utf8",
);
...
async function main() {
  ...
  console.log("Applying property type categories migration…");
  ...
  if (url && key) {
    ...
    const probe = await admin.from("properties").select("property_type").limit(1);
    if (probe.error) {
      console.warn("Probe after migration:", probe.error.message);
    }
  }

  console.log("✓ Property type categories migration applied.");
}
