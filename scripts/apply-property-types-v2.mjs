/**
 * Apply bnb/hotel/villa types + commercial minimum lease column.
 * Usage: npm run db:migrate:property-types-v2
 */
...
const MIGRATION_SQL = readFileSync(
  join(root, "supabase", "migrations", "20260709180000_property_types_commercial_lease.sql"),
  "utf8",
);
...
async function main() {
  ...
  console.log("Applying property types v2 + commercial lease migration…");
  ...
  if (url && key) {
    ...
    const probe = await admin
      .from("properties")
      .select("property_type, minimum_rent_period_months")
      .limit(1);
    if (probe.error) {
  ...
  console.log("✓ Property types v2 migration applied.");
}
