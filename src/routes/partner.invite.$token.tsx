...
export const Route = createFileRoute("/partner/invite/$token")({
  head: () => ({ meta: [{ title: "Pilot invitation — NyumbaSearch" }] }),
...
function PartnerInvitePage() {
  ...
  const invitePath = `/partner/invite/${token}`;

  const accept = useMutation({
    mutationFn: async () => {
      const { acceptPilotInvitation } = await import("@/lib/api/pilot-partnership.functions");
      return acceptPilotInvitation({ data: { token } });
    },
    onSuccess: () => {
