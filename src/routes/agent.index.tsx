...
export const Route = createFileRoute("/agent/")({
  head: () =>
    buildPageHead({
      title: "Real Estate Agent Portal — NyumbaSearch",
      description:
        "List client properties, manage leads, and grow your pipeline as a real estate agent on NyumbaSearch.",
      path: "/agent",
    }),
...
function AgentEntry() {
  const { user, isAgent, loading, pendingApplications } = useAuth();
  ...
  useEffect(() => {
    ...
    if (user && hasPendingApplicationForRole(pendingApplications, "agent")) {
      navigate({ to: "/auth/pending" });
    }
  }, [user, isAgent, loading, pendingApplications, navigate]);
      ...
      <div className="mx-auto grid min-h-screen max-w-6xl gap-12 px-6 py-10 lg:grid-cols-2 lg:py-16">
        <div className="flex flex-col">
          ...
          <div className="mt-6 flex-1">
            <div className="inline-flex items-center gap-2 rounded-full border border-background/20 bg-background/10 px-3 py-1 text-xs font-medium">
              <UserRound className="h-3 w-3 text-gold" /> Real Estate Agent Portal
            </div>
            <h1 className="mt-6 font-display text-4xl font-semibold leading-tight sm:text-5xl">
              Sell and let on <span className="text-gold">NyumbaSearch</span>.
            </h1>
            <p className="mt-5 max-w-md text-background/75">
              List client homes, capture tenant and buyer leads, and manage your book after ops
              approval.
            </p>
              ...
              {[
                {
                  icon: UserRound,
                  t: "Agent listings",
                  d: "Publish and manage properties for your clients.",
                },
                {
                  icon: TrendingUp,
                  t: "Lead pipeline",
                  d: "Track inquiries and grow closings.",
                },
              ].map((f) => (
                <div
                  key={f.t}
        ...
        <div className="flex items-center">
          <div className="w-full rounded-3xl border border-background/15 bg-background p-8 text-foreground shadow-2xl">
            <h2 className="font-display text-2xl font-semibold">Apply as real estate agent</h2>
            <p className="mt-2 text-sm text-muted-foreground">
              Sign up with your trading name. After ops approval you&apos;ll land on the agent
              dashboard with listing and lead tools.
            </p>
            <Link
              to="/auth"
              search={{ signupFor: "agent", mode: "signup", redirect: "/agent/dashboard" }}
            ...
            <Link
              to="/auth"
              search={{ signupFor: "agent", mode: "signin", redirect: "/agent/dashboard" }}
