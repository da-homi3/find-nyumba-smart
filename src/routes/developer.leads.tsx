...
export const Route = createFileRoute("/developer/leads")({
...
function Page() {
  ...
  const statusMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["developer-leads"] });
  ...
  const openThread = (id: string) => {
    setActiveThread(id);
    navigate({ to: "/developer/leads", search: { thread: id } });
  };

  const closeThread = () => {
    setActiveThread(undefined);
    navigate({ to: "/developer/leads", search: { thread: undefined } });
  };
  ...
  return (
    <div className="px-6 py-8 lg:px-10">
      <h1 className="font-display text-3xl font-semibold">Developer leads</h1>
      <p className="text-sm text-muted-foreground">{leads.length} inquiries across portfolio</p>
      <LeadPackUpgradeBanner portal="property_developer" />
      <PortalLeadsList
        isLoading={isLoading}
        leads={leads}
        userId={user?.id}
