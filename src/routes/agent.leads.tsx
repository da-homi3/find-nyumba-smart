...
function Page() {
  ...
  const openThread = (id: string) => {
    setActiveThread(id);
    navigate({ to: "/agent/leads", search: { thread: id } });
  };

  const closeThread = () => {
    setActiveThread(undefined);
    navigate({ to: "/agent/leads", search: { thread: undefined } });
  };
  ...
  return (
    <div className="px-6 py-8 lg:px-10">
      <h1 className="font-display text-3xl font-semibold">Agent leads</h1>
      <p className="text-sm text-muted-foreground">{leads.length} inquiries across portfolio</p>
      <LeadPackUpgradeBanner portal="agent" />
      <PortalLeadsList
        isLoading={isLoading}
        leads={leads}
        userId={user?.id}
