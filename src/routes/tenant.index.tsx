...
const neighborhoods = ["All", "Kilimani", "Westlands", "Karen", "Lavington", "Kileleshwa", "Kasarani", "South B", "Roysambu"];
...
function TenantHome() {
  ...
  const filtered = properties.filter((p) => {
    ...
    const matchQ = !q || p.title.toLowerCase().includes(q.toLowerCase()) || p.neighborhood.toLowerCase().includes(q.toLowerCase());
  ...
  return (
    <div>
      {/* Hero strip */}
      <header className="bg-gradient-emerald px-5 pt-10 pb-20 text-primary-foreground">
        <div className="mx-auto max-w-2xl">
          <p className="text-xs font-medium uppercase tracking-wider text-primary-foreground/70">Karibu</p>
          <h1 className="mt-1 font-display text-3xl font-semibold leading-tight">Find your next home in Nairobi</h1>
      ...
      {verified.length > 0 && (
        <section className="mx-auto max-w-2xl px-5 pt-6">
          <div className="flex items-center justify-between">
            ...
            <Link to="/tenant/map" className="text-xs font-medium text-primary">Map view →</Link>
          </div>
      ...
      <section className="mx-auto max-w-2xl px-5 pt-6">
        <div className="flex items-start gap-3 rounded-2xl border bg-gradient-to-br from-accent to-secondary p-4">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">"Find me a 1BR under 40k near Westlands with reliable water."</p>
          </div>
        </div>
      </section>
