...
export function PortalBillingPage({ portal }: Readonly<{ portal: ListingPortal }>) {
  const paths = PORTAL_PATHS[portal];
  const { data: payments = [] } = useQuery<BillingPayment[]>({
    queryKey: ["billing-payments", portal],
  ...
  return (
    <div className="px-6 py-8 lg:px-10">
      ...
      <div className="mt-8 rounded-2xl border bg-card p-6">
        <p className="text-sm text-muted-foreground">Paid recently (last 3 completed)</p>
        <p className="mt-1 font-display text-2xl font-semibold">{formatKes(recentPaid || 0)}</p>
        <button
          type="button"
