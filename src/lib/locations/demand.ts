...
export async function loadLocationDemand(
  ...
  const [
    ...
    admin
      ...
      .gte("created_at", since)
      .limit(5000),
    admin.from("property_views").select("property_id").gte("created_at", since).limit(8000),
    admin
      .from("properties")
      .select("location_id")
  ...
  for (const s of searches ?? []) {
    ...
    const hood = normalizeLocationName(
      String((s as { neighborhood?: string | null }).neighborhood ?? ""),
    );
  ...
  for (const s of locSearches ?? []) {
    ...
    const q = normalizeLocationName(
      String((s as { normalized_query?: string | null }).normalized_query ?? ""),
    );
  ...
  const propertyIds = [
    ...new Set((views ?? []).map((v) => (v as { property_id: string }).property_id)),
  ];
  if (propertyIds.length) {
    const chunk = propertyIds.slice(0, 1500);
    const { data: props } = await admin.from("properties").select("id,location_id").in("id", chunk);
  ...
  const rows = [...byId.values()].map((row) => {
    const demandScore =
      row.searchCount * 3 +
      row.viewCount * 2 +
      row.inventoryCount * 1.5 -
      row.needsReviewCount * 0.5;
