...
export async function flagSuspiciousReferral(
  ...
  const [{ data: referrer }, { data: referred }] = await Promise.all([
    admin.from("profiles").select("phone, full_name").eq("id", referral.referrer_user_id).maybeSingle(),
    admin.from("profiles").select("phone, full_name").eq("id", referral.referred_user_id).maybeSingle(),
  ]);

  const suspicious =
    (referrer?.phone && referred?.phone && referrer.phone === referred.phone);

  if (suspicious) {
    await db
      .from("referrals")
      .update({ status: "fraud_flagged" })
      .eq("id", referral.id);
...
export async function referrerAtCap(admin: Admin, referrerId: string): Promise<boolean> {
  ...
  const { count } = await (admin as SupabaseClient<any>)
    .from("referrals")
