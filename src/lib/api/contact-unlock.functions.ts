...
async function resolveContactPhone(
  ...
  const direct = property.contact_phone?.trim();
...
export const getListingUnlockState = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    if (unlock || isPlus) {
      contactPhone = await resolveContactPhone(supabaseAdmin, data.listingId);
    }
...
export const unlockListingContact = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const contactPhone = await resolveContactPhone(supabaseAdmin, data.listingId);
    ...
    if (paymentRes.status === "completed") {
      const phone = await resolveContactPhone(supabaseAdmin, data.listingId);
      return { unlocked: true, contactPhone: phone, method: "paid" as const };
    }
