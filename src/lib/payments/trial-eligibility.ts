...
/** Paid cycle length, plus one free month on the first successful subscription payment (lister/provider only). */
export async function subscriptionPeriodDaysAfterPayment(
  ...
  if (paymentType === "tenant_plus" || (paymentType && !isTrialEligiblePaymentType(paymentType))) {
    return { days: base, bonusFreeMonth: false };
  }
